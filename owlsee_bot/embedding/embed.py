...
for file in tqdm(
    os.listdir("text/" + DOMAIN + "/"), "Processing the data source files..."
):
...
df["n_tokens"] = tqdm(
    df.text.apply(lambda x: len(tokenizer.encode(x))), "Tokenizing..."
)
...
for row in tqdm(df.iterrows(), "Chunking data from rows..."):
...
df["n_tokens"] = tqdm(df.text.apply(lambda x: len(tokenizer.encode(x))), "Encoding...")

# generate the data embeddings
df["embeddings"] = tqdm(
    df.text.apply(
